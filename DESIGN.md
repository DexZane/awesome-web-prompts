...
当 AI 编程助手（如 Cursor, Copilot, Cline 等）读取本文件时，应当遵循以下核心设计美学与技术约束，从而避免生成千牌一律的“模板化”或“廉价感”网页。
...
- **Physical Depth & Volumetric 3D (实体物理厚度与空间感)**：UI 组件不再局限于扁平化，通过多层 Z 轴 `translateZ` 堆叠与 3D 视差倾斜（Parallax Tilt）构建真实的物理空间感。
...
- **[DO]** 使用基于 HSL 或精心调配的色板（如深紫灰 `#0c0a16`、近乎纯黑 `#050505`，配合霓虹亮色 `#dbff00` 或纯黑背景搭配 `#F6FCFF` 高亮文字）。
...
- **优先选用的字体**：
  ...
  - 极客/等宽字体（Monospace）：JetBrains Mono, Courier New, 或其他特殊风格字体（如 Qanelas-Heavy）。
- **排版技巧**：
  ...
  - **SVG 响应式对齐 (SVG ViewBox Vector Alignment Trick)**：对于需要突破标准 CSS 字体尺寸限制、完美贴合容器左右边界的超大品牌标题文本，采用内联 `<svg>` + `viewBox="0 0 X Y"` + `<text>`，保证在任意屏幕分辨率下自适应拉伸且不折行。
...
- **[DO]** 实体切片厚度 (Volumetric 3D Layering)：在 3D 转换场景中，通过平行堆叠 5+ 层不同 Z 轴偏移（`translateZ(-1.5px)` 至 `translateZ(1.5px)`）的微层，构建真实的实体卡片侧边厚度。
...
- **鼠标跟随视差与惯性衰减（3D Parallax & Inertia Damping）**：监听 `mousemove` 事件，计算光标相对于窗口中心的偏移量，利用线性插值（`val += (target - val) * damping`）实现平滑跟随的 3D 卡片/视角倾斜。
- **60fps 原生数学动画循环 (Smoothstep & requestAnimationFrame)**：对于无依赖卡片圆柱轮播、磁吸悬停等高级场景，使用原生 `requestAnimationFrame` + `smoothstep` 阶梯插值函数进行精准位置求解。
- **交错阶梯进场（Staggered Entrance Animation）**：通过 CSS 自定义延迟类（如 `.delay-100` 至 `.delay-500`）或 JavaScript 索引计算 `transitionDelay: `${350 + index * 50}ms``，实现极具韵律感的元素逐个浮现。
- **磁性吸附（Magnetic Hover）**：光标靠近按钮时，按钮和内部文本产生跟随偏移。

## 7. 技术栈约束 (Tech Stack Constraints)

- **框架**：React 18+ (或 Next.js), TypeScript, Vite.
- **样式**：Tailwind CSS v3 / Tailwind CSS v4。优先使用原子类，但遇到复杂阴影、遮罩、3D 空间 `perspective`、`transformStyle: preserve-3d` 或自定义字体时，可在 `index.css` 中手写原生 CSS。
- **动画引擎**：原生 CSS Transitions / Framer Motion (用于组件级交互与进场) / GSAP + ScrollTrigger (用于复杂的滚动劫持、深度视差和时间轴动画)。
- **图标**：统一使用 `lucide-react` 或 FontAwesome CDN，并精确控制其尺寸（如 `w-5 h-5`）、描边宽度（`stroke-width 1.5`）与透明度。
